...
export const loadLanguageData = async (language: SupportedLanguage): Promise<IFlexiResume> => {
  ...
  try {
    ...
    debugDataLoader.extend('error')(`Failed to load data for language: ${language}`, error);
  ...
  await Promise.all(
    languages.map(async (language) => {
      try {
        ...
        debugDataLoader.extend('error')(`Failed to preload data for language: ${language}`, error);
      }
    })
  );
};
...
export const initializeLanguage = (): void => {
  try {
    ...
    debugDataLoader.extend('error')('Failed to read language preference from localStorage', error);
    currentLanguage = detectBrowserLanguage();
  }
};
...
export const saveLanguagePreference = (language: SupportedLanguage): void => {
  try {
    ...
    debugDataLoader.extend('error')('Failed to save language preference to localStorage', error);
  }
};
