...
export const defaultProjectConfig: ProjectConfig = {
  cdn: {
    ...
    autoRemoval: {
      enabled: parseEnvBoolean(import.meta.env?.VITE_CDN_AUTO_REMOVAL_ENABLED, true),
      failureThreshold: parseEnvNumber(import.meta.env?.VITE_CDN_FAILURE_THRESHOLD, 3),
      minCDNCount: parseEnvNumber(import.meta.env?.VITE_CDN_MIN_COUNT, 1),
      recheckInterval: parseEnvNumber(import.meta.env?.VITE_CDN_RECHECK_INTERVAL, 300000), // 5分钟
      recoveryStrategy: (import.meta.env?.VITE_CDN_RECOVERY_STRATEGY as 'manual' | 'auto' | 'scheduled') || 'manual',
      successRateThreshold: parseEnvNumber(import.meta.env?.VITE_CDN_SUCCESS_RATE_THRESHOLD, 20), // 20%成功率阈值
    },
    localOptimization: {
...
export const isDevelopment = (): boolean => {
  ...
  // 根据强制环境模式返回结果
  switch (forceEnvMode.toLowerCase()) {
    case 'development':
      return true;
    case 'production':
      return false;
...
export interface CDNConfig {
  ...
  /** CDN自动移除策略 / CDN automatic removal strategy */
  autoRemoval: {
    /** 是否启用自动移除 / Whether to enable automatic removal */
    enabled: boolean;

    /** 连续失败次数阈值 / Consecutive failure count threshold */
    failureThreshold: number;

    /** 最小保留CDN数量 / Minimum CDN count to keep */
    minCDNCount: number;

    /** 重新检测间隔(毫秒) / Recheck interval in milliseconds */
    recheckInterval: number;

    /** 移除后的恢复策略 / Recovery strategy after removal */
    recoveryStrategy: 'manual' | 'auto' | 'scheduled';

    /** 成功率阈值(0-100)，低于此值的CDN将被移除 / Success rate threshold (0-100), CDNs below this will be removed */
