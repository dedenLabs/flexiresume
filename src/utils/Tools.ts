...
import { globalCache, imageCache } from '../utils/MemoryManager';
...
const initializeDataCache = async () => {
    ...
    initPromise = (async () => {
        try {
            ...
            debugCache.extend('error')('Failed to initialize data cache: %O', error);
        } finally {
...
export const updateDataCache = async (): Promise<void> => {
    try {
        ...
        debugCache.extend('error')('Failed to update data cache: %O', error);
    }
};
...
export function useLazyVideo() {
    const loadVideo = async (videoEl) => {
        ...
        try {
            // 使用CDN管理器的本地URL构建逻辑
            localFallbackUrl = await cdnManager.getResourceUrl(sources.original, {
                enableFallback: true,
                localBasePath: '',
            ...
            // 如果CDN管理器失败，保持原始URL
            debugCDN.extend('error')('Failed to build local fallback URL: %O', error);
        }
        if (isDevelopment()) {
 ...
 */
export async function replaceCDNBaseURL(url: string) {
    try {
        // 移除基础路径，只保留相对路径
        const relativePath = cdnManager.extractResourcePath(url);
        let cache = imageCache.get(relativePath);
        if (cache) return cache.url;
        return await cdnManager.getResourceUrl(relativePath, {
            enableFallback: true,
            localBasePath: '',
            cacheUrls: true,
        });
    } catch (error) {
        debugCDN.extend('error')('Failed to get CDN URL, using original: %O', error);
        return url;
    }
}

/**
 * 同步版本的CDN URL替换（用于CSS等同步场景）
 * Sync version of CDN URL replacement (for CSS and other sync scenarios)
 */
export function replaceCDNBaseURLSync(url: string) {
    try {
        // 移除基础路径，只保留相对路径
        const relativePath = cdnManager.extractResourcePath(url);
        let cache = imageCache.get(relativePath);
        ...
        // 同步版本：直接使用最佳CDN，不进行验证
        const bestCDN = cdnHealthChecker.getBestCDN();
        if (bestCDN) {
            return cdnManager.buildCDNUrl(bestCDN, relativePath);
        }
        
        return url;
    } catch (error) {
        debugCDN.extend('error')('Failed to get CDN URL sync, using original: %O', error);
        return url;
    }
}