...
import {
  getCurrentPositionNameByPath,
  updateCurrentResumeStore,
  watchMinWidth,
} from "../utils/Tools";
...
const FlexiResume: React.FC<FlexiResumeProps> = ({ path }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        try {
          debugResume(
            `触发音频播放: ${path} (上次路径: ${previousPathRef.current})`,
          );
  ...
  return (
    <ResumeWrapper
      ...
      style={{ minWidth: `${minWidth - 40}px`, maxWidth: `800px` }}
    >
        ...
        switch (m.type) {
          ...
          case "project_experience":
            return (
              <Section
                key={i}
                title={`${m.name}${m?.list?.length > 1 ? "（" + m?.list?.length + "）" : ""}`}
                {...args}
              >
